...
- **Deterministic core** (`src/logic/`): a topologically-sorted combinational-circuit simulator with cycle/floating-input/multiple-driver detection, a truth-table generator, and a known-function validator — plain TypeScript, unit-tested (90+ vitest cases), with zero dependency on the UI or on any LLM. Digital logic correctness is never delegated to a language model.
