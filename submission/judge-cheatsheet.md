...
- `src/logic/simulator.ts` and `src/logic/validators.ts` — the deterministic circuit engine; `src/logic/*.test.ts` — 52+ passing tests. Digital logic is never computed by an LLM here, only acted on.
...
| Execution | 52+ unit tests on the deterministic core, CI on every push, deployed build |
